(() => {
  ...
  document.addEventListener("DOMContentLoaded", function() {
    ...
    if (calendarGrid && currentMonthLabel) {
      let renderCalendar2 = function() {
        ...
        for (let day = 1; day <= lastDay.getDate(); day++) {
          ...
          if (thisDate < today) {
            ...
            if (thisDate.getDay() === 0) {
              ...
              dayEl.addEventListener("click", () => selectDate2(thisDate, dayEl));
            }
          }
      ...
      }, selectDate2 = function(date, element) {
        selectedDate = date;
        selectedTime = null;
        ...
        if (bookingSummaryEl)
          bookingSummaryEl.style.display = "none";
      };
      var renderCalendar = renderCalendar2, selectDate = selectDate2;
...
Wir werden Sie zur Bestatigung kontaktieren.`);
    ...
    if (baComparison && baSlider && baBeforeImage && baAfterImage) {
      let updateBAProject2 = function(index) {
        ...
        if (baProjectTitle)
          baProjectTitle.textContent = project.title;
        if (baProjectDescription)
          baProjectDescription.textContent = project.description;
        if (baProjectTags) {
          baProjectTags.innerHTML = project.tags.map(
            (tag) => `<span class="ba-tag">${tag}</span>`
          ).join("");
        }
        ...
        if (baPrevBtn)
          baPrevBtn.disabled = index === 0;
        if (baNextBtn)
          baNextBtn.disabled = index === baProjects.length - 1;
      }, initBADots2 = function() {
        if (!baDotsContainer)
          return;
        baDotsContainer.innerHTML = baProjects.map(
          (_, i) => `<div class="ba-dot ${i === 0 ? "active" : ""}" data-index="${i}"></div>`
        ).join("");
        document.querySelectorAll(".ba-dot").forEach((dot) => {
          dot.addEventListener("click", () => {
            currentBAIndex = parseInt(dot.dataset.index);
            updateBAProject2(currentBAIndex);
          });
        });
      }, updateSliderPosition2 = function(x) {
      ...
      var updateBAProject = updateBAProject2, initBADots = initBADots2, updateSliderPosition = updateSliderPosition2;
      const baProjects = [
        {
      ...
      document.addEventListener("mousemove", (e) => {
        if (isDragging)
          updateSliderPosition2(e.clientX);
      });
      ...
      document.addEventListener("touchmove", (e) => {
        if (isDragging)
          updateSliderPosition2(e.touches[0].clientX);
      });
      document.addEventListener("touchend", () => {
        isDragging = false;
      });
      baComparison.addEventListener("click", (e) => {
        if (e.target === baSlider || e.target.parentElement === baSlider)
          return;
        updateSliderPosition2(e.clientX);
      });
      initBADots2();
    ...
    if (uploadArea && uploadInput) {
      let handleFiles2 = function(files) {
      ...
      var handleFiles = handleFiles2, displayPreview = displayPreview2, removeImage = removeImage2, updateUploadCount = updateUploadCount2;
      let uploadedFiles = [];
      const MAX_FILES = 10;
      ...
      uploadInput.addEventListener("change", (e) => {
        handleFiles2(Array.from(e.target.files));
      });
      window.removeImage = removeImage2;
    }
    ...
    if (entsorgenUploadArea && entsorgenInput) {
      let checkImageDimensions2 = function(file) {
      ...
      }, displayEntsorgenPreview2 = function(file, dimensions) {
        const reader = new FileReader();
        reader.onload = (e) => {
          const previewItem = document.createElement("div");
          previewItem.className = "preview-item";
          ...
          if (entsorgenPreviewGrid)
            entsorgenPreviewGrid.appendChild(previewItem);
        };
        reader.readAsDataURL(file);
      }, removeEntsorgenImage2 = function(filename) {
        ...
        updateEntsorgenCount2();
      }, updateEntsorgenCount2 = function() {
        if (!entsorgenUploadCountEl)
          return;
        if (entsorgenFiles.length > 0) {
          ...
          if (entsorgenFiles.length >= MIN_ENTSORGEN_FILES) {
            showQualityWarning2(`Perfekt! Sie haben ${entsorgenFiles.length} qualitativ hochwertige Bilder hochgeladen.`, "success");
          }
        } else {
          entsorgenUploadCountEl.innerHTML = "";
        }
      }, showQualityWarning2 = function(message, type) {
        if (!qualityWarningEl)
          return;
      ...
      var checkImageDimensions = checkImageDimensions2, displayEntsorgenPreview = displayEntsorgenPreview2, removeEntsorgenImage = removeEntsorgenImage2, updateEntsorgenCount = updateEntsorgenCount2, showQualityWarning = showQualityWarning2;
      let entsorgenFiles = [];
      ...
      async function handleEntsorgenFiles(files) {
        ...
        if (entsorgenFiles.length + files.length > MAX_ENTSORGEN_FILES) {
          showQualityWarning2(`Sie konnen maximal ${MAX_ENTSORGEN_FILES} Bilder hochladen`, "error");
          return;
        }
        for (const file of files) {
          if (!file.type.startsWith("image/")) {
            showQualityWarning2(`${file.name} ist keine Bilddatei`, "error");
            continue;
          }
          if (file.size < MIN_FILE_SIZE) {
            showQualityWarning2(`${file.name} ist zu klein (min. 200KB). Bitte hochwertigere Bilder hochladen.`, "error");
            continue;
          }
          const dimensions = await checkImageDimensions2(file);
          if (!dimensions) {
            showQualityWarning2(`${file.name} konnte nicht geladen werden`, "error");
            continue;
          }
          ...
          displayEntsorgenPreview2(file, dimensions);
        }
        updateEntsorgenCount2();
      }
      ...
      if (entsorgenForm) {
        entsorgenForm.addEventListener("submit", (e) => {
          e.preventDefault();
          if (entsorgenFiles.length < MIN_ENTSORGEN_FILES) {
            showQualityWarning2(`Bitte laden Sie mindestens ${MIN_ENTSORGEN_FILES} qualitativ hochwertige Bilder hoch.`, "error");
            window.scrollTo({ top: entsorgenUploadArea.offsetTop - 100, behavior: "smooth" });
            return;
          }
          showQualityWarning2("Vielen Dank! Ihre Anfrage wird verarbeitet. Wir melden uns innerhalb von 24 Stunden bei Ihnen.", "success");
          setTimeout(() => {
            ...
            entsorgenFiles = [];
            if (entsorgenPreviewGrid)
              entsorgenPreviewGrid.innerHTML = "";
            updateEntsorgenCount2();
            if (qualityWarningEl)
              qualityWarningEl.style.display = "none";
          }, 1e3);
      ...
      window.removeEntsorgenImage = removeEntsorgenImage2;
    }
    class Slider {
      constructor(wrapperId, prevBtnId, nextBtnId, dotsId, options = {}) {
        ...
        this.autoPlayInterval = options.autoPlayInterval || 5e3;
    ...
    new Slider(
      ...
      "galleryNext",
      "galleryDots",
      { autoPlay: true, autoPlayInterval: 6e3 }
    );
    new Slider(
      ...
      "testimonialsNext",
      "testimonialsDots",
      { autoPlay: true, autoPlayInterval: 7e3 }
    );
    ...
    const servicesData = {
      gartengestaltung: {
        ...
        features: [
          { icon: "\u{1F4D0}", title: "Individuelle Planung", text: "Massgeschneiderte Konzepte nach Ihren Wunschen und den ortlichen Gegebenheiten" },
          { icon: "\u{1F3A8}", title: "Kreatives Design", text: "Moderne und zeitlose Gartengestaltung mit Stil" },
          { icon: "\u{1F331}", title: "Bepflanzung", text: "Auswahl passender Pflanzen fur jeden Standort" },
          { icon: "\u{1F4A7}", title: "Bewasserung", text: "Automatische Bewasserungssysteme auf Wunsch" }
        ],
        pricing: [
          { label: "Basis", value: "50-80\u20AC/m\xB2" },
          { label: "Standard", value: "80-150\u20AC/m\xB2" },
          { label: "Premium", value: "150-300\u20AC/m\xB2" }
        ]
      },
      gartenpflege: {
        icon: "\u{1F33F}",
        ...
        features: [
          ...
          { icon: "\u{1F333}", title: "Hecken & Straucher", text: "Fachgerechter Schnitt zur richtigen Jahreszeit" },
          { icon: "\u{1F5D3}\uFE0F", title: "Abo-Service", text: "Regelmassige Pflege nach festem Zeitplan" },
          { icon: "\u{1F342}", title: "Saisonarbeiten", text: "Herbstlaub, Winterschutz, Fruhjahrsputz" }
        ],
        pricing: [
      ...
      pflasterung: {
        ...
        features: [
          { icon: "\u{1F3D7}\uFE0F", title: "Terrassen", text: "Naturstein, Beton oder Keramikplatten" },
          { icon: "\u{1F697}", title: "Einfahrten", text: "Belastbare Pflasterung fur Fahrzeuge" },
          { icon: "\u{1F6E4}\uFE0F", title: "Gartenwege", text: "Funktionale und dekorative Wegefuhrung" },
          { icon: "\u{1F4A1}", title: "Beleuchtung", text: "Integration von LED-Beleuchtung moglich" }
        ],
        pricing: [
          { label: "Betonstein", value: "60-90\u20AC/m\xB2" },
          { label: "Naturstein", value: "90-150\u20AC/m\xB2" },
          { label: "Premium", value: "150-250\u20AC/m\xB2" }
        ]
      },
      rollrasen: {
        icon: "\u{1F69C}",
        ...
        features: [
          { icon: "\u{1F331}", title: "Premium-Qualitat", text: "Hochwertige Rasensorten fur jeden Standort" },
          { icon: "\u26A1", title: "Schnell nutzbar", text: "Nach 2-3 Wochen voll belastbar" },
          { icon: "\u{1F4CF}", title: "Bodenvorbereitung", text: "Professionelle Vorbereitung des Untergrunds" },
          { icon: "\u{1F4A7}", title: "Erstpflege", text: "Anleitung und Tipps fur optimales Anwachsen" }
        ],
        pricing: [
          { label: "Basis", value: "15-20\u20AC/m\xB2" },
          { label: "Premium", value: "20-30\u20AC/m\xB2" },
          { label: "Mit Sprinkler", value: "35-45\u20AC/m\xB2" }
        ]
      },
      zaunbau: {
        icon: "\u{1F3E0}",
        ...
        features: [
          { icon: "\u{1FAB5}", title: "Holzzaune", text: "Naturliche und warme Optik" },
          { icon: "\u{1F527}", title: "Metallzaune", text: "Robust und pflegeleicht" },
          { icon: "\u{1F38B}", title: "Sichtschutz", text: "Elemente aus Holz, Metall oder Bambus" },
          { icon: "\u{1F512}", title: "Tore & Turen", text: "Passende Zugangslosungen" }
        ],
        pricing: [
      ...
      winterdienst: {
        ...
        features: [
          { icon: "\u{1F69C}", title: "Schneeraumung", text: "Schnelle Raumung bei Schneefall" },
          { icon: "\u{1F9C2}", title: "Streudienst", text: "Professionelles Streuen gegen Glatte" },
          { icon: "\u{1F4DE}", title: "Bereitschaft", text: "24/7 erreichbar wahrend Winterperiode" },
          { icon: "\u{1F4C5}", title: "Vertragsservice", text: "Saisonvertrage fur Gewerbe und Privat" }
        ],
        pricing: [
...
Wir werden uns innerhalb von 24 Stunden bei Ihnen melden.`);
    ...
    function nextAppointmentStep(stepNumber) {
      ...
      if (terminEl)
        terminEl.scrollIntoView({ behavior: "smooth", block: "start" });
      if (stepNumber === 4)
    ...
    if (heroEl) {
      ...
      var throttle = throttle2, parallaxEffect = parallaxEffect2;
      window.addEventListener("scroll", throttle2(parallaxEffect2, 10));
    }
    ...
    function updateContactStepper() {
      ...
      if (prevBtn)
        prevBtn.style.display = currentContactStep === 1 ? "none" : "block";
      if (nextBtn)
        nextBtn.style.display = currentContactStep === totalContactSteps ? "none" : "block";
      if (submitBtn)
        submitBtn.style.display = currentContactStep === totalContactSteps ? "block" : "none";
    }
